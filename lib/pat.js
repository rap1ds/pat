      ...
      var matches = patterns.map(function(pattern) {
        ...
        if(pattern.all) {
          ...
          if(matchAllResult.isSome()) {
            return some({pattern: pattern, args: matchAllResult.val()});
          } else {
            return none();
          }
        } else {
          ...
          if(firstArgsResult.isSome() && restArgsResult.isSome()) {
            ...
            if(rest) {
              return some({pattern: pattern, args: firstArgsResult.val().concat([rest])});
            } else {
              return some({pattern: pattern, args: firstArgsResult.val()});
            }
          } else {
            return none();
          }
